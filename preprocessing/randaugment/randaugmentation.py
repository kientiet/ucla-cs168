'''
  This is adopted from Google UDA Team Authors
'''
...
def get_mean_and_std(dataset):
  if dataset == "cifar-10":
    means = [0.4913997551666284, 0.48215855929893703, 0.4465309133731618]
    stds = [0.24703225141799082, 0.24348516474564, 0.26158783926049628]
  else:
    means = [0., 0., 0.]
    stds = [1., 1., 1.]
  return means, stds
...
def pil_wrap(img, dataset, use_mean_std):
  """Convert the `img` numpy tensor to a PIL Image."""

  if use_mean_std:
    MEANS, STDS = get_mean_and_std(dataset)
  else:
    MEANS = [0, 0, 0]
    STDS = [1, 1, 1]
...
def pil_unwrap(pil_img, dataset, use_mean_std, img_shape):
...
def apply_policy(policy, img, dataset, use_mean_std=True):
  ...
  """
  img_shape = img.shape
  pil_img = pil_wrap(img, dataset, use_mean_std)
  for xform in policy:
    assert len(xform) == 3
  ...
  return pil_unwrap(pil_img, dataset, use_mean_std, img_shape)
