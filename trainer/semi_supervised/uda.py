...
class SemiTrainer():
  def __init__(self,
    ...
    self.optimizer = optim.SGD(self.model.parameters(), lr = self.base_lr, weight_decay = 8e-3, momentum = 0.9)
    ...
    # Apply EMA
    self.ema = ExponentialMovingAverage(self.model, device, num_updates = True)
      ...
      for phase in ["train", "eval"]:
        ...
        for sup_images, labels in tqdm(dataloader, total = len(dataloader)):
          ...
          if phase == "train":
            ...
            with torch.no_grad():
              val_loss, logits = self.calc_loss(sup_images, labels, eval_model = new_model, eval_mode = True)
  ...
  def calc_loss(self, sup_images, labels, eval_mode, eval_model = None, ori_images = None, aug_images = None, current_step = None):
    model = self.model
    if eval_model is not None:
      model = eval_model

    if eval_mode:
      val_loss, logits = self.loss_func(sup_images, labels, model, eval_mode = eval_mode)
      return val_loss, logits

    else:
      semi_loss, supervised_loss, unsup_loss, anneal_threshold = self.loss_func(sup_images, labels, model, ori_images, aug_images,
